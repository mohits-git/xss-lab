...
func (q *Queries) CreateComment(blogID, userID int, content string) (*Comment, error) {
...
func (q *Queries) GetCommentByID(commentID int) (*Comment, error) {
	query := "SELECT id, blog_id, user_id, content, created_at FROM comments WHERE id = ?"
	row := q.db.QueryRow(query, commentID)

	var comment Comment
	err := row.Scan(&comment.ID, &comment.BlogID, &comment.UserID, &comment.Content, &comment.CreatedAt)
	if err != nil {
...
func (q *Queries) GetCommentsByBlogID(blogID int) ([]*Comment, error) {
	query := "SELECT id, blog_id, user_id, content, created_at FROM comments WHERE blog_id = ?"
